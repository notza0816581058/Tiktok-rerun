import { mockAccountId, mockComments, mockLiveSessionId, mockProducts, mockStats } from "./fixtures";
import type {
  AnyRequest,
  ClientErrorCode,
  ClientResult,
...
function validLive(payload: { accountId: string; liveSessionId: string }): ClientResult<never> | null {
  return validAccount(payload) ??
...
export function createMockTransport(): TikTokTransport {
  ...
  return {
    async request<K extends Operation>(input: RequestFor<K>): Promise<ClientResult<OperationMap[K]["response"]>> {
      ...
      switch (request.operation) {
        case "auth.status": {
          result = validAccount(request.payload) ?? success({
        ...
        case "product.search": {
          ...
          result = error ??
            (limit < 1 || limit > 100
              ...
              : success({
                  products: mockProducts.filter((product) => product.title.toLowerCase().includes(query))
        ...
        case "product.pin": {
          ...
          const productExists = mockProducts.some((product) => product.id === request.payload.productId);
          if (error) {
            result = error;
          } else if (!productExists) {
            result = failure("PRODUCT_NOT_FOUND", "The sample product does not exist.");
          } else if (request.operation === "product.pin" && !addedProducts.has(request.payload.productId)) {
        ...
        case "comment.list": {
          result = validLive(request.payload) ?? success({
