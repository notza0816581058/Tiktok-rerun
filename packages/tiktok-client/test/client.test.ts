...
import { createMockTransport, createTikTokClient, mockAccountId, mockLiveSessionId } from '../src';
...
test('mock modules return normalized fixtures marked pending verification', async () => {
  ...
  assert.equal(stats.ok && stats.data.sold, 19);
...
test('invalid mock identifiers and input return typed errors', async () => {
  const client = createTikTokClient(createMockTransport());
  const missingAccount = await client.stats.live({
    accountId: 'missing',
    liveSessionId: mockLiveSessionId,
  });
  const missingLive = await client.comments.list({
    accountId: mockAccountId,
    liveSessionId: 'missing',
  });
  const missingProduct = await client.products.add({ ...live, productId: 'missing' });
  const invalidSearch = await client.products.search({
    accountId: mockAccountId,
    query: '',
    limit: 0,
  });
